# Retinal Fundus Images Dataset Codebook

## Overview

This codebook provides detailed specifications for the Retinal Fundus Images dataset, including field definitions, value ranges, clinical interpretations, and harmonization mappings for integration into the ophthalmology data harmonization pipeline.
...
|------------|-----------|-------------|----------|------------------|
| image_id | string | Unique identifier for each fundus image | Yes | Format: `{patient_id}_{eye}_{severity_level}_{image_number}` |
| patient_id | string | Anonymous patient identifier | Yes | Unique per patient |
| eye | categorical | Eye laterality | Yes | OD, OS |
| severity_level | categorical | DR severity classification | Yes | no_dr, mild, moderate, severe, proliferative |
| image_number | integer | Image sequence number | No | Range: 1-10 |
...
#### No DR
- **Clinical Definition**: No diabetic retinopathy present
...
|-----------|-------|------|-------------|
| resolution_x | 1024 | pixels | Horizontal resolution |
| resolution_y | 1024 | pixels | Vertical resolution |
